<?php
 ...
 // var_dump($_SESSION['jugador']);
  ...
  if(!isset($_SESSION['jugador'])){
    header('Location: https://phpprojects-victorvte.c9.io/MathDiceMini.3/index.php');
  }else{
    $juego = new Juego();
    //dado de 6
      ...
      //calcular
        ...
        if(isset($_POST['dado0'])){
          if($_POST['dado0'] == $_SESSION["calcular"]){
            ...
            $updatePuntos = $db->updatePuntos($_SESSION['jugador']->getPuntos(1), $_SESSION['jugador']->getNombre(), $_SESSION['jugador']->getApellido());
            //var_dump($updatePuntos);
            if($_SESSION['jugador']->getPuntos() == $_SESSION['jugador']->getMeta()){
              echo '<script>
                      var c = confirm("Enhorabuena!! Has ganado! Quieres  seguir jugando?");
                        if (c == true) {
                            window.location.replace("https://phpprojects-victorvte.c9.io/MathDiceMini.3/junior.php");
                        } else {
                            window.location.replace("https://phpprojects-victorvte.c9.io/MathDiceMini.3/index.php");
                        }
                    </script>';
